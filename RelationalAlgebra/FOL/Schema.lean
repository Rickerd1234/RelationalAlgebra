...
def BoundedQuery.hasSafeTerm {n : ℕ} (t : fol.Term (Attribute ⊕ Fin n)) : (q : BoundedQuery dbs n) → Prop
  | .R _ vMap => ∃i, (vMap i) = t
...
@[simp]
theorem BoundedQuery.hasSafeTerm.R_def {n : ℕ} {dbs : DatabaseSchema} (t : Fin (dbs rn).card → fol.Term (Attribute ⊕ Fin n)) (t' : fol.Term (Attribute ⊕ Fin n)) :
  (R rn t).hasSafeTerm t' = ∃i, (t i) = t' := by rfl

@[simp]
theorem BoundedQuery.hasSafeTerm.tEq_def {n : ℕ} (q : BoundedQuery dbs n) (t₁ t₂ : fol.Term (Attribute ⊕ Fin n)) (t' : fol.Term (Attribute ⊕ Fin n))  :
  (tEq q t₁ t₂).hasSafeTerm t' = q.hasSafeTerm t' := by rfl

@[simp]
theorem BoundedQuery.hasSafeTerm.and_def {n : ℕ} (q₁ q₂ : BoundedQuery dbs n) (t' : fol.Term (Attribute ⊕ Fin n)) :
  (and q₁ q₂).hasSafeTerm t' = (q₁.hasSafeTerm t' ∨ q₂.hasSafeTerm t') := by rfl

@[simp]
theorem BoundedQuery.hasSafeTerm.ex_def {n : ℕ} (q : BoundedQuery dbs (n + 1)) (t' : fol.Term (Attribute ⊕ Fin n)) :
  (ex q).hasSafeTerm t' = q.hasSafeTerm (t'.relabel (Sum.map id (Fin.castLE (Nat.le_add_right n 1)))) := by rfl

@[simp]
theorem BoundedQuery.hasSafeTerm.or_def {n : ℕ} (q₁ q₂ : BoundedQuery dbs n) (t' : fol.Term (Attribute ⊕ Fin n)) :
  (or q₁ q₂).hasSafeTerm t' = (q₁.hasSafeTerm t' ∨ q₂.hasSafeTerm t') := by rfl

@[simp]
theorem BoundedQuery.hasSafeTerm.not_def {n : ℕ} (q : BoundedQuery dbs n) (t' : fol.Term (Attribute ⊕ Fin n)) :
  (not q).hasSafeTerm t' = q.hasSafeTerm t' := by rfl

-- schema of query
def BoundedQuery.schema {n : ℕ} : (q : BoundedQuery dbs n) → Finset Attribute
  | .R name vMap => (R name vMap).toFormula.freeVarFinset
  | .tEq q _ _ => q.schema
...
@[simp]
theorem BoundedQuery.hasSafeTerm_mem_schema (q : BoundedQuery dbs n) :
...
@[simp]
theorem BoundedQuery.hasSafeTerm_ex_Fin (q : BoundedQuery dbs n) :
...
@[simp]
theorem BoundedQuery.hasSafeTerm_ext_schema {q₁ q₂ : BoundedQuery dbs n} (h : (∀t, q₁.hasSafeTerm t ↔ q₂.hasSafeTerm t)) :
