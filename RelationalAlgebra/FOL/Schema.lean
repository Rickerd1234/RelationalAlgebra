...
variable {ρ α : Type} {dbs : ρ → Finset String}
...
@[simp]
theorem BoundedQuery.schema.R_def_mem {n : ℕ} {x : String} (t : Fin (dbs rn).card → (fol dbs).Term (String ⊕ Fin n)) :
...
instance BoundedQuery.schema.R_fintype {rn} {t : Fin (dbs rn).card → (fol dbs).Term (String ⊕ Fin n)} :
...
@[simp]
theorem BoundedQuery.schema.R_def {n : ℕ} (t : Fin (dbs rn).card → (fol dbs).Term (String ⊕ Fin n)) :
