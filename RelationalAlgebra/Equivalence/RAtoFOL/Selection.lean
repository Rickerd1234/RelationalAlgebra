  ...
  (ih: ∀t, (ra_to_fol_query q dbi.schema).RealizeDom dbi t → t ∈ RA.Query.evaluateT dbi q) :
    ∀t, (ra_to_fol_query (.s a b p q) dbi.schema).RealizeDom dbi t → t ∈ RA.Query.evaluateT dbi (.s a b p q) := by
      ...
      simp only [RA.Query.isWellTyped.s_def, ra_to_fol_query, FOL.outVar.def,
        FOL.Query.RealizeDom.def, FOL.BoundedQuery.Realize.def, FOL.BoundedQuery.toFormula_tEq,
      ...
      intro a_1 a_2 a_3
      simp_all only [FOL.Query.RealizeDom.def, ra_to_fol_query.isWellTyped, and_imp]
      obtain ⟨left, right⟩ := h
