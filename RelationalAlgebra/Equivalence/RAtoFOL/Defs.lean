...
noncomputable def ra_to_fol_query_def [FOL.folStruc] (raQ : RA.Query) (dbs : DatabaseSchema) : FOL.Query :=
  ...
  | .s a b pos sq => (ra_to_fol_query_def sq dbs).relabel (λ a' => Sum.inl (ite (a' = att_to_var b) (att_to_var a) a'))
...
noncomputable def ra_to_fol_outFn_def [FOL.folStruc] (raQ : RA.Query) (dbs : DatabaseSchema) : Attribute →. FOL.Variable :=
  match raQ with
  | .R rn => PFun.res att_to_var (dbs rn)
  | .s a b pos sq => (λ a' => (ra_to_fol_outFn_def sq dbs) (ite (a' = b) a a'))
  | .p rs sq => ra_to_fol_outFn_p (ra_to_fol_outFn_def sq dbs) rs
  | .j sq1 sq2 => sorry -- @TODO
  | .r f sq => ra_to_fol_outFn_r (ra_to_fol_outFn_def sq dbs) f
  ...
  by
    ...
    case p rs sq ih =>
      ...
      · intro a
        ...
        exact (ra_to_fol_outFn_schema_mem_iff left).mpr (right a)
      · intro a
        obtain ⟨w, h⟩ := a
        split at h
        next h_1 => simp_all only
